...
void MainWindow::setupNodesDisplay()
{
    ...
    if(outputString!=""){
        ...
        QTreeWidgetItem *exchanger = new QTreeWidgetItem(ui.treeWidget_nodeViewer);
        ...
        for (int i = 0 ;i<nodesList.size()-1;i++){
            ...
            nodeItem->setText(0,nodesList[i][0]);
            nodeItem->setText(1,nodesList[i][1].split(" = ")[1]);
        }
...
void MainWindow::setupCPUInfo()
{
    ...
    //    client->executeShellCommand("cat /proc/cpuinfo",outputString);
    //    get cores
    //            QRegularExpression re("(?<=^CPU\(s\):)(.*)");
    ...
    //    if (match.hasMatch()) {
    //        QString cores = match.captured(0).replace(" ","");
    //        ui.label_CPUCoreShow->setText(cores);
    //    }
}
